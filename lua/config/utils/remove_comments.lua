...
local comment_chars = {
  ...
  typescript = '//',
  typescriptreact = '//',
  lua = '%-%-', -- Ya escapado para Lua pattern
  python = '#',
  yaml = '#',
  bash = '#',
  sh = '#',
...
M.remove_inline = function()
  ...
  if changed > 0 then
    ...
    vim.notify(('Eliminados %d comentarios inline'):format(changed), vim.log.levels.INFO)
  else
...
M.remove_all = function()
  ...
  if removed > 0 then
    ...
    vim.notify(('Eliminadas %d líneas de comentarios'):format(removed), vim.log.levels.INFO)
  else
