...
local function run_preview(source_buf, output_buf)
  ...
  vim.fn.jobstart(cmd, {
    ...
    on_exit = function(_, code)
      ...
      write_to_buf(output_buf, { '', '─────────────────────────────', status }, true)
    end,
  })
end
...
M.start = function()
  ...
  local group = vim.api.nvim_create_augroup('Preview_' .. source_buf, { clear = true })
  ...
  vim.notify('Preview activado. Guarda el archivo para ejecutar.', vim.log.levels.INFO)
