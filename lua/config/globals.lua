...
lk.toggle_loc = function()
  ...
  for _, win in ipairs(vim.api.nvim_list_wins()) do
    if vim.fn.getloclist(vim.api.nvim_win_get_number(win), { filewinid = 0 }).filewinid > 0 then
      loc_open = true
      break
    end
  end
...
lk.get_highest_diagnostic_severity = function()
  local dominated = { vim.diagnostic.severity.ERROR, vim.diagnostic.severity.WARN, vim.diagnostic.severity.INFO, vim.diagnostic.severity.HINT }
